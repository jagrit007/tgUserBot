...
async def kang(args):
    ...
    if not args.text[0].isalpha() and args.text[0] not in ("/", "#", "@", "!"):
        ...
        if photo:
            ...
            packname = f"b{user.id}_by_{user.username}_{pack}"
            ...
            if "  A <strong>Telegram</strong> user has created the <strong>Sticker&nbsp;Set</strong>." not in htmlstr:
                ...
                await args.edit("Sticker pack doesn't exist! Making a new one!")
                async with bot.conversation('Stickers') as conv:
                    await conv.send_message('/newpack')
                    await conv.get_response()
                    # Ensure user doesn't get spamming notifications
                    await bot.send_read_acknowledge(conv.chat_id)
                    await conv.send_message(f"@{user.username}'s kanged pack")
                    await conv.get_response()
                    # Ensure user doesn't get spamming notifications
                    await bot.send_read_acknowledge(conv.chat_id)
                    file.seek(0)
